...
def ctr_eval(sess, model, data, batch_size):
    ...
    while start + batch_size <= data.shape[0]:
        auc, f1, pre, recall = model.eval(sess, get_feed_dict(model, data, start, start + batch_size))
        auc_list.append(auc)
        f1_list.append(f1)
        pre_list.append(pre)
        recall_list.append(recall)
        start += batch_size
    return float(np.mean(auc_list)), float(np.mean(f1_list)),float(np.mean(pre_list)),float(np.mean(recall_list))
