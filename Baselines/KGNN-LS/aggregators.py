...
class Aggregator(object):
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        if not avg:
            ...
            user_relation_scores_normalized = tf.nn.softmax(user_relation_scores, axis=-1) #本来是dim改为axis
...
class SumAggregator(Aggregator):
    def __init__(self, batch_size, dim, dropout=0.8, act=tf.nn.relu, name=None):  #这里dropput本来是0. 改成0.8了，这样下面rate就会变成0.2
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings, masks):
        ...
        output = tf.nn.dropout(output, rate=1-self.dropout) #本来是keep_prob，改为rate
