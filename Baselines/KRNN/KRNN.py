...
from Dataset import KRNNDataset
...
def train(args):
    ...
    train_dataset = KRNNDataset(x_train)
    validation_dataset = KRNNDataset(x_validation)
    test_dataset = KRNNDataset(x_test)
    ...
    net = KRNN(num_user, num_entity, num_relation, kg, args, device).to(device)
        ...
        with torch.no_grad():
            ...
            for user_ids, item_ids, labels in tqdm(validation_loader, total=validation_loader_len, desc='val'):
                ...
                try:
                    ...
                    total_precision += precision_score(labels.cpu().detach().numpy(),pre_lable.cpu().detach().numpy(), average='binary')
                    total_recall += recall_score(labels.cpu().detach().numpy(),pre_lable.cpu().detach().numpy(), average='binary')
                except ValueError:
                    pass
        ...
        with torch.no_grad():
            ...
            for user_ids, item_ids, labels in tqdm(test_loader, total=test_loader_len, desc='test'):
                ...
                try:
                    ...
                    total_t_f1 += f1_score(labels.cpu().detach().numpy(), pre_lable.cpu().detach().numpy(), average='binary')
                    total_t_precision += precision_score(labels.cpu().detach().numpy(), pre_lable.cpu().detach().numpy(), average='binary')
                    total_t_recall += recall_score(labels.cpu().detach().numpy(), pre_lable.cpu().detach().numpy(), average='binary')
                except ValueError:
                    pass
