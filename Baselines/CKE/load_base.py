...
def load_kg(data_dir):
    ...
    relation_set = set()
    entity_set = set()
    for edge in tqdm(edges, desc='load_KG'):
        head = edge[0]
        tail = edge[1]
        relation = edge[2]
...
def data_split(ratings_np, ratio):
    ...
    for user in tqdm(positive_records, desc='data_split'):
...
def convert_dict(ratings_np):
    ...
    for pair in tqdm(ratings_np, desc='convert_KG'):
        user = pair[0]
        item = pair[1]
        label = pair[2]
        if label == 1:
...
def get_records(data_set):
    records = dict()
    for pair in tqdm(data_set, desc='get record'):
        user = pair[0]
        item = pair[1]
        label = pair[2]
        if label == 1:
...
def load_data(args):
    ...
    n_user = len(user_set)
    n_item = len(item_set)
    data = [n_entity, n_user, n_item, n_relation, train_set, eval_set, test_set, kg_dict, rec]
